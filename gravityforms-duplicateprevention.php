...
class GravityFormsDuplicatePrevention {
	...
	/**
	 * Set logging supported.
	 *
	 * @param array $plugins An array with plugins.
	 *
	 * @return array
	 */
	 ...
	 *
	 * @param array $validation_result Results as an array.
	 *
	 * @return string
	 */
	protected function format_log( $validation_result ) {
			...
			if ( $field instanceof GF_Field_CreditCard ) {
				foreach ( range( 1, 5 ) as $input_id ) {
					...
					if ( isset( $input[ $input_name ] ) ) {
						// $form, $lead_id, and $lead params not used for credit card.
						$input[ $input_name ] = $field->get_value_save_entry(
							$input[ $input_name ],
							null,
							$input_name,
							null,
		...
		return sprintf(
			'Blocking duplicate submission for form ID %d: %s',
			$validation_result['form']['id'],
			print_r( $input, true )
		);
	}
	...
	public function load_script() {
		wp_register_script(
			'gform-duplicateprevention',
			plugins_url( 'gravityforms-duplicateprevention.js', __FILE__ ),
			array( 'jquery' ),
			self::PLUGIN_VERSION,
			true
		);
